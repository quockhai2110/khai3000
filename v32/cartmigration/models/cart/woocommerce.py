...
class LeCartWoocommerce(LeCartWordpress):
	...
	def category_import(self, convert, category, categories_ext):
		...
		term_id = post_data_id = self.import_product_data_connector(self.create_insert_query_connector('terms',terms_data),True,convert['id'])

		# Them vao bang Term meta
		termmeta_data = {
			'term_id': term_id,
			'meta_key': 'thumbnail_id',
			'meta_value': 0,
		...
		self.import_product_data_connector(self.create_insert_query_connector('termmeta',termmeta_data),True,convert['id'])

		# Them vao bang Term taxonomy
		termtaxonomy_data = {
			'term_id': term_id,
		...
		termtaxonomy_id = self.import_product_data_connector(self.create_insert_query_connector('term_taxonomy',termtaxonomy_data),True,convert['id'])
		self.insert_map(map_type= self.TYPE_CATEGORY, id_desc = termtaxonomy_id, id_src=convert['id'])


		return response_success(termtaxonomy_id)
	...
	def product_import(self, convert, product, products_ext):
		...
		post_data_id = self.import_product_data_connector(self.create_insert_query_connector('posts',post_data),True,convert['id'])
		...
		# if convert['thumb_image']['url'] or convert['thumb_image']['path']:
		# 	image_process = self.process_image_before_import(convert['thumb_image']['url'], convert['thumb_image']['path'])
		# 	image_import_path = self.uploadImageConnector(image_process, self.add_prefix_path(self.make_woocommerce_image_path(image_process['path'], self.TYPE_PRODUCT),'wp-content/uploads'.rstrip('/')))
		# 	if image_import_path:
		# 		cate_image = self.remove_prefix_path(image_import_path, 'wp-content/uploads'.rstrip('/'))
		# 		image_details = self.get_sizes(image_process['url'])
		# 		thumbnail_id = self.wp_image(cate_image, image_details)
		# if thumbnail_id:
		...
		# 	}
		#
		# 	product_update_query = self.create_update_query_connector('postmeta', postmeta_image_value, where_cons)
		# 	product_update_id = self.import_data_connector(product_update_query, self.TYPE_PRODUCT, post_data_id)
		# 	if not product_update_id:
		# 		return response_warning('product id ' + to_str(post_data_id) + ' update image fails.')
		#


		#Them vao bang postmeta
		meta_key = {
		...
		for key, value in meta_key.items():
			...
			product_import = self.import_product_data_connector(self.create_insert_query_connector('postmeta',postmeta),True, convert['id'])
		#
		#
		#
		#
		# #Them vao bang post_meta_lookup
		# product_meta_lookup = {
		...
		# }
		#
		# product_import = self.import_product_data_connector(self.create_insert_query_connector('wc_product_meta_lookup', product_meta_lookup), True,convert['id'])
		products_to_categories = products_ext['data']['products_to_categories']

		for i in range(0,40):
			if convert['id'] == products_to_categories[i]['products_id']:
				categories_id_src = products_to_categories[i]['categories_id']
				break;

		categories_id_desc = self.get_map_field_by_src(map_type=self.TYPE_CATEGORY, id_src = categories_id_src)

		termrelationships_data = {
			'object_id': post_data_id,#product_id
			'term_taxonomy_id': categories_id_desc,

		}
		self.import_product_data_connector(self.create_insert_query_connector('term_relationships', termrelationships_data), True,convert['id'])

		self.insert_map(map_type=self.TYPE_PRODUCT, id_desc=post_data_id, code_src = convert['name'], id_src = convert['id'])
		return response_success(product_import)
	...
	def customer_import(self, convert, customer, customers_ext):
		customer_users = {
			...
			'user_url': '',
			'user_registered': int(time.time()),
			'user_activation_key': '',
			'user_status': 0,
			'display_name': convert['first_name'] + convert['middle_name'] + convert['last_name'],
		}
		customer_id = self.import_customer_data_connector(self.create_insert_query_connector('users', customer_users), True, convert['id'])
		...
		self.import_customer_data_connector(self.create_insert_query_connector('wc_customer_lookup', customer_lookup), True, convert['id'])
	...
	def order_import(self, convert, order, orders_ext):
		...
		post_id = self.import_order_data_connector(self.create_insert_query_connector('posts',post_data),True,convert['id'])

		order_data_shipping = convert['shipping_address']
		order_data_billing = convert['billing_address']
		postmeta_data = {
			'_shipping_country': order_data_shipping['country']['name'],
			'_shipping_postcode': order_data_shipping['postcode'],
			'_shipping_state': order_data_shipping['state']['name'],
			'_shipping_city': order_data_shipping['city'],
			'_shipping_address2': order_data_shipping['address_2'],
			'_shipping_address1': order_data_shipping['address_1'],
			'_shipping_company': order_data_shipping['company'],
			'_shipping_last_name': order_data_shipping['last_name'],
			'_shipping_first_name': order_data_shipping['first_name'],
			'_billing_phone': order_data_billing['telephone'],
			'_billing_email': '',
			'_billing_country': order_data_billing['country']['name'],
			'_billing_postcode': order_data_billing['postcode'],
			'_billing_state': order_data_billing['state']['name'],
			'_billing_city': order_data_billing['city'],
			'_billing_address2': order_data_billing['address_2'],
			'_billing_address1': order_data_billing['address_1'],
			'_billing_company': order_data_billing['company'],
			'_billing_last_name': order_data_billing['last_name'],
			'_billing_first_name': order_data_billing['first_name'],
			'_created_via': 'admin',
			'_order_key': '',
		...
		for key,value in postmeta_data.items():
			...
			order_id = self.import_order_data_connector(self.create_insert_query_connector('postmeta', postmeta), True,convert['id'])

		orderstats_data = {
			'order_id': post_id,
			'parent_id': 0,
			'date_created': datetime.datetime.now(),
			'date_created_gmt': datetime.datetime.now(),
			'num_items_sold': 0,
			'total_sales': 0,
			'tax_total': 0,
			'shipping_total': 0,
		...
		self.import_order_data_connector(self.create_insert_query_connector('wc_order_stats', orderstats_data), True, convert['id'])

		order_product = orders_ext['data']['orders_products']
		order_total = orders_ext['data']['orders_total']
		# woocommerceorderitem_data = {
		# 	'order_item_name': order_product[convert['id'] - 1]['products_name'],
		# 	'order_item_type': 'line_item',
		# 	'order_id': post_id,
		# }

		# Them cac product vao trong order, Trong Order co the co nhieu product
		# for i in range(0, len(order_product)):
		# 	if convert['id'] == order_product[i]['orders_id']:
		# 		woocommerceorderitem_data = {
		# 			'order_item_name': order_product[i]['products_name'],
		# 		 	'order_item_type': 'line_item',
		# 			'order_id': post_id,
		# 		}
		# 		order_item_id = self.import_order_data_connector(self.create_insert_query_connector('woocommerce_order_items',woocommerceorderitem_data),True, convert['id'])
		#
		# 		woocommerceorderitemmeta_metakey_data = {
		...
		# 		}
		#
		# 		for key, value in woocommerceorderitemmeta_metakey_data.items():
		# 			woocommerceorderitemmeta_data = {
		# 				'order_item_id': order_item_id,
		# 				'meta_key': key,
		# 				'meta_value': value,
		# 			}
		# 			self.import_order_data_connector(self.create_insert_query_connector('woocommerce_order_itemmeta',woocommerceorderitemmeta_data),True, convert['id'])
		#

		#xu ly tung product trong phan order
		item = convert['items']
		for i in range(0, len(item)):
			woocommerceorderitem_data = {
				'order_item_name': item[i]['product']['name'],
				'order_item_type': 'line_item',
				'order_id': post_id,
			}
			order_item_id = self.import_order_data_connector(self.create_insert_query_connector('woocommerce_order_items',woocommerceorderitem_data),True, convert['id'])
			...
			woocommerceorderitemmeta_metakey_data = {
				'_line_tax_data': 'a:2:{s:8:"subtotal";a:0:{}s:5:"total";a:0:{}}',
				'_line_tax': 0,
				'_line_total': item[i]['price'],
				'_line_subtotal_tax': convert['items'][i]['total'],
				'_line_subtotal': convert['items'][i]['subtotal'],
				'_tax_class': '',
				'_qty': item[i]['qty'],
				'_variation_id': 0,
				'_product_id': 5,
		...
		#Tinh shipping
		#Them vao bang woocommercorderitem
		#Them vao bang woocommercorderitem_meta
		woocommerceorderitem_data = {
			'order_item_name': 'Shipping',
			'order_item_type': 'shipping',
			'order_id': post_id,
		}
		order_shipping = self.import_order_data_connector(self.create_insert_query_connector('woocommerce_order_items', woocommerceorderitem_data), True,convert['id'])
