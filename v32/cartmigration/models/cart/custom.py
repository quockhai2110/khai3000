...
class LeCartCustom(LeBasecart):
	...
	def get_categories_ext_export(self, categories):
		...
		categories_ext_queries = {
			...
			'products_to_categories': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_products_to_categories WHERE categories_id ORDER BY categories_id"
			},
		}
	...
	def get_products_ext_export(self, products):
		...
		product_ext_queries = {
			...
			'products_description': {
			 ...
			 'products_to_categories': {
			 	'type': 'select',
			 	'query': "SELECT * FROM _DBPRF_products_to_categories WHERE products_id ORDER BY categories_id",
			 },
			# 'manufacturers': {
			# 	'type': 'select',
	...
	def convert_product_export(self, product, products_ext):
		...
		product_data['created_at'] = product['products_date_added'] if product['products_date_added'] else get_current_time()
		product_data['updated_at'] = product['products_last_modified'] if product['products_last_modified'] else product_data['created_at']
		...
		#product_data['description'] = html.unescape(products_ext_data['products_description'])
		#product_description = get_list_from_list_by_field(products_ext_data['products_description'], 'product_id',product['products_id'])
	...
	def get_orders_ext_export(self, orders):
		...
		orders_ext_queries = {
			'orders_products': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_orders_products WHERE orders_id"
			},
			'orders_total': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_orders_total WHERE orders_id ORDER BY orders_id "
			},
			# 'orders_payment': {
			# 	'type': 'select',
			...
			# 'countries': {
			#  	'type': 'select',
			#  	'query': "SELECT * FROM _DBPRF_countries WHERE countries_id IN " + self.list_to_in_condition(countries_id)
			# },
			# #
			# 'currencies': {
			#  	'type': 'select',
			#  	'query': "SELECT * FROM  _DBPRF_currencies WHERE currencies_id IN " + self.list_to_in_condition(currencies_ids)
			# }
		}
	...
	def convert_order_export(self, order, orders_ext):
		...
		#Lay gia tri cac hang cua bang order_total  co cung oder_id
		order_total_id = get_list_from_list_by_field(orders_ext['data']['orders_total'], 'orders_id', order['orders_id'])

		#Lay gia tri 1 hang cua order_total_id sao cho class = 'ot_tax'
		ot_tax = get_row_from_list_by_field(order_total_id, 'class', 'ot_tax')
		ot_shipping = get_row_from_list_by_field(order_total_id, 'class', 'ot_shipping')
		ot_subtotal = get_row_from_list_by_field(order_total_id, 'class', 'ot_subtotal')
		ot_total = get_row_from_list_by_field(order_total_id, 'class', 'ot_total')
		if ot_tax:
		...
		order_data['shipping']['title'] = 'Shipping'
		order_data['shipping']['amount'] = get_value_by_key_in_dict(ot_shipping, 'value', 0.0)
		order_data['total']['title'] = 'Total'
		order_data['total']['amount'] = get_value_by_key_in_dict(ot_total, 'value', 0.0)
		order_data['subtotal']['title'] = 'Total products'
		order_data['subtotal']['amount'] = get_value_by_key_in_dict(ot_subtotal, 'value', 0.0)
		...
		#order_customer la o trong bang Order
		...
		# 	'billing_state'])
		# if billing_state:
		# 	order_billing['state']['id'] = billing_state['zone_code']
		...
		import re
		pharse = order['delivery_name']
		matches =re.findall('\w+',pharse)
		...
		order_delivery['first_name'] = matches[0]
		order_delivery['last_name'] = matches[1]
		...
		# delivery_country = get_row_from_list_by_field(order['countries'], 'alpha', order['ship_cntry'])
		# if delivery_country:
		...
		#payments = get_row_from_list_by_field(order['payment_method'], 'order_id', order['id'])
